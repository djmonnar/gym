...
- 진입 흐름(스플래시, 3단계 온보딩, 체험 로그인, 위치 권한 안내): 앱 크롬 없는 전체 화면 랜딩, 데스크톱 좌우 분할·모바일 세로 스택
...
- 딥그린·올리브 액센트·웜화이트 기반 리턴패스 브랜드와 리턴샵 명칭 적용
...
- 진입 흐름을 앱 목업 배경 대신 딥그린 타이포그래피 랜딩으로 재구성하고, 스플래시·온보딩·로그인·위치에서 인앱 헤더와 하단 탭을 감춰 첫인상 화면과 서비스 화면을 분리
...
src/
  ...
  components/entry/       진입 흐름 랜딩(공용 레이아웃, 스플래시, 온보딩, 로그인, 위치)
...
scripts/
  ...
  check-color-classes.mjs  색상 클래스가 실제로 컴파일됐는지 검사(빌드에 포함)
```

다음 단계에서는 신규 screen id와 역할별 라우팅을 추가하고, 이후 Firebase 구현 저장소가 `ReturnPassRepository` 인터페이스를 구현하도록 교체합니다.
- 형광 라임을 걷어내고 배경 밝기에 따라 액센트를 두 단계로 분리(밝은 배경  / 어두운 배경 ), 네온 글로우 그림자와 의미 없는 강조 타일 제거
- Tailwind가 존재하지 않는 색상 클래스를 조용히 무시해 엉뚱한 색으로 폴백되는 문제를 빌드에서 차단(
> returnpass-ui-prototype@0.1.0 check:classes
> node scripts/check-color-classes.mjs

색상 클래스 검사 통과: 모든 클래스가 CSS에 존재합니다.)
